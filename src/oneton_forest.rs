use idtree;
use storage::{Storage, HierarchicalOneToNStorage, AnyStorage, IntoIter, IntoIterMut};
use ::DenseVec;
...
impl<'a, T: 'a> Storage<'a, T> for OneToNForest<T>{
    ...
    fn insert(&mut self, guid: usize, t: T){
        let id = self.arena.new_node(t).id();
...
unsafe impl<T> AnyStorage for OneToNForest<T>{}
